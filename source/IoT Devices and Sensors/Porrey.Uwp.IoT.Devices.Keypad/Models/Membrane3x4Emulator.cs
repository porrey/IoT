...
{
	public class Membrane3x4Emulator : KeyPad
	{
		private Timer _timer = null;
		private Random _rnd = new Random();

		public Membrane3x4Emulator()
			: base(0, new IGpioPinMapping[0])
		{
			_timer = new Timer(this.TimerCallback, null, 1000, Timeout.Infinite);
		}
		...
		private async void TimerCallback(object state)
		{
			try
			{
				int i = _rnd.Next(0, 11);
				ICharacterGridMapping mapping = this.CharacterGridMappings[i];

				this.OnKeyEvent(new KeyPadEventArgs(mapping.Value, KeyDownEventType.ButtonDown));

				if (this.EnableButtonUpEvent)
				{
					await Task.Delay(250);
					this.OnKeyEvent(new KeyPadEventArgs(mapping.Value, KeyDownEventType.ButtonUp));
				}
			}
			finally
			{
